...
class DroneClient {
    constructor(config) {
        ...
        this._eventSource = (url) => {
            return new EventSource(config.baseUrl+url, {
                headers: {
                    Authorization: `Bearer ${config.token}`
                }
            });
    ...
    encryptRepoSecret(owner, name, ymlContent) {

        Joi.assert(owner, Joi.string().required(), 'Must specify owner');
        Joi.assert(name, Joi.string().required(), 'Must specify name');
        return this._request('post', `/api/repos/${owner}/${name}/encrypt`, { payload: ymlContent });
    };
    ...
    getBuildLogsEventSource(owner, name, num, job) {

        Joi.assert(owner, Joi.string().required(), 'Must specify owner');
        Joi.assert(name, Joi.string().required(), 'Must specify name');
        Joi.assert(num, Joi.number().required(), 'Must specify num');
        Joi.assert(job, Joi.number().required(), 'Must specify job');
        return this._eventSource(`/api/stream/${owner}/${name}/${num}/${job}`);
    }
